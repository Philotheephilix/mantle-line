...
contract MNTPriceOracle {
    ...
    modifier onlySubmitter() {
        require(msg.sender == submitter, "MNTPriceOracle: caller is not the submitter");
        _;
    }
    
    modifier onlyOwner() {
        require(msg.sender == owner, "MNTPriceOracle: caller is not the owner");
        _;
    }
    ...
    constructor(address _submitter) {
        require(_submitter != address(0), "MNTPriceOracle: submitter is zero address");
    ...
    function getWindowsInRange(uint256 start, uint256 end) external view returns (uint256[] memory) {
        require(start <= end, "MNTPriceOracle: invalid range");
    ...
    function updateSubmitter(address newSubmitter) external onlyOwner {
        require(newSubmitter != address(0), "MNTPriceOracle: new submitter is zero address");
    ...
    function transferOwnership(address newOwner) external onlyOwner {
        require(newOwner != address(0), "MNTPriceOracle: new owner is zero address");
