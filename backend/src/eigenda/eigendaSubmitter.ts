...
export class EigenDASubmitter {
  ...
  /**
   * Retrieve data from EigenDA using a commitment
   */
  public async retrieveData(commitment: string): Promise<PriceWindowPayload> {
    ...
    const payload = JSON.parse(dataJson) as PriceWindowPayload;
